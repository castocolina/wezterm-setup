...
#
# Env overrides (testing): WEZTERM_BOOTSTRAP_PREFIX, WEZTERM_BIN_DIR, WEZTERM_APP_DIR.
set -euo pipefail
...
# ${WEZTERM_APP_DIR:-${HOME}/Applications} (user-path only — never the system
# /Applications, never sudo, never hdiutil/DMG — D-05). Version selection (the
# `tag` arg) is fed by the SAME resolve_want_datestamp + select_release path the
# Linux branch uses, so both OSes track the same nightly contract (D-05).
#
...
install_macos() {
  local tag="$1"
  local app_dir="${WEZTERM_APP_DIR:-${HOME}/Applications}"
  ...
  # Place under ${WEZTERM_APP_DIR:-${HOME}/Applications} (user-path only — never
  # /Applications, never sudo). Fresh placement: rm -rf the prior bundle then
  # cp -R the new one.
  mkdir -p "${app_dir}"
