...
*Last updated: 2026-06-23 — Phase 7.1 (07.1-03) flipped macOS deviation #5 (`wez keys --json`) + #6 (scene PATH) to fixed-in-07.1. Scene-PATH symlink fixed in 07.1 (Plan 07.1-01, `install_macos`): `install_macos` now symlinks `~/.local/bin/wezterm` → the bundle CLI so `wez scene new`/`launch` resolve `wezterm` out of the box on a clean macOS install (no longer the "non-numeric pane id" failure; mirrors `install_linux`). The `wez keys --json` dkjson require-path bug is fixed in 07.1 (Plan 07.1-02, dual-resolve `cli.vendor.dkjson` + regression) — cross-platform; the shipped v1.0.0 binary still carries it (fix reaches end users via a new release). INST-08 remains DEFERRED/non-gating (the arm64 end-user first-launch self-check ships in 07.1-03 as `tools/silicon-check.sh`; INST-08's macOS status flips only when a real-Silicon run is pasted into the runbook). The Coverage table agrees byte-for-byte with the REQUIREMENTS.md Traceability table per ID. Prior: 2026-06-22 — Phase 7 (07-05) flipped the 13 platform-sensitive D-18 requirement IDs from the deferred qualifier to the "macOS verified D-18 (§...)" form on the green harness evidence (verify-macos.sh PASS=26 FAIL=0 + agent-driven runbook + §5/§6 ui-ux PASS); SCEN-01/02 additionally flipped to "macOS verified D-18 (§6)". 2026-06-20 — reconciled the Coverage table with delivered reality (TAB-01..05, SCEN-01/02, INST-07/08/09 flipped from Pending to Done). 2026-06-14 — added Phase 6 (Ergonomic Installer, INST-07) + Phase 7 (macOS Parity Pass, D-18); discuss-phase 6 added INST-08.*
