...
> Per D-09 the entire phase runs UNATTENDED — there is no human-check task; the
> stable `v*` tag is auto-pushed green-gated (D-10) and the runbook is agent-driven
> via a tmux/wezterm-cli/screencapture harness (D-12).
...
| P04-T1 | 07-04 | 2 | INST-07 | T-07-13/15 | dispatch on living branch (--ref); unattended CI wait; +branchname asset; log-driven re-dispatch loop | live CI | `gh run watch <id> --exit-status` | ✅ (release.yml after W1) | ⬜ pending |
| P04-T2 | 07-04 | 2 | INST-07/INST-06 | T-07-13/14/16 | checksum-verified-before-chmod; D-07 verify-then-decide; +branchname version match; doctor exit 0 | live E2E | `wez doctor` (exit 0) ; quarantine probe | ✅ (install path after W1) | ⬜ pending |
| P04-T3 | 07-04 | 2 | INST-07 | T-07-17 | first v* tag green-gated auto-push (dry-run-green AND E2E-green); no human gate | live CI | `git push origin vX.Y.Z` then `gh run watch <id> --exit-status` (=0) — guarded by dry-run+E2E green | ✅ (release.yml stable path) | ⬜ pending |
| P05-T1 | 07-05 | 3 | INST-01/FOUND-01/DIAG-05/PANE-01..04/SCEN-03..06 | T-07-18/19 | auto gate FAIL=0; runbook agent-driven (tmux/wezterm-cli/screencapture); ui-ux notes | gate + runbook | `bash tools/verify-macos.sh` (FAIL=0, exit 0) | ✅ (verify-macos.sh) | ⬜ pending |
| P05-T2 | 07-05 | 3 | (all D-18 IDs) | T-07-19 | each Done flip cites a runbook section; tables agree per ID; flipped autonomously | doc-consistency | `for id in INST-01 INST-06 INST-07 FOUND-01 DIAG-05 PANE-01..04 SCEN-03..06; do grep -F "$id" REQUIREMENTS.md ROADMAP.md \| grep -c 'deferred D-18'; done` (per-ID gate, each =0) | ✅ (REQUIREMENTS/ROADMAP) | ⬜ pending |

*Status: ⬜ pending · ✅ green · ❌ red · ⚠️ flaky. Sampling continuity: no 3 consecutive tasks lack an automated/live verify — every task above carries an automated command or a live-CI/E2E verify (P04-T3 is now a green-gated auto-push confirmed via `gh run watch --exit-status`, not a human-check). The phase is fully unattended (D-09): there is no remaining human-check task.*
...
|----------|-------------|------------|-------------------|
| Visual identity parity (tab-bar accents, pane bg, emoji cell-width, colors/glyphs) | PANE/TAB/SCEN visual sections | Visual judgment — but NO human sign-off (D-12): the `agent-ui-ux-designer` subagent judges from `screencapture` screenshots | Drive runbook visual sections (§5/§6) agent-driven via the tmux/wezterm-cli/screencapture harness; capture `agent-ui-ux-designer` notes per D-03/D-12 (Plan 05 Task 1) |
| CWD inheritance (OSC 7) on macOS shells | FOUND-01 | Needs a live WezTerm session (driven by the agent via `wezterm cli`) | Runbook §3 — split pane / new tab inherits the active pane cwd (via `wezterm cli split-pane`/`spawn`); recorded repro (Plan 05 Task 1) |
| Gatekeeper first-launch + quarantine on this Intel Mac | INST-06/07 | Needs the live macOS install (driven by the agent) | Plan 04 Task 2 — E2E install + `xattr -p com.apple.quarantine` probe (D-07) |
...
> Note: the first public `v*` tag push is NO LONGER a manual verification — per D-10 it is an autonomous green-gated auto-push (gated on the dispatch dry-run AND the E2E install both being green), confirmed via `gh run watch <id> --exit-status` (Plan 04 Task 3).

---

## Validation Sign-Off

- [x] All tasks have `<automated>` verify or Wave 0 dependencies (no human-check task remains — P04-T3 is now a green-gated auto-push confirmed via live CI, per D-09/D-10)
...
**Approval:** planned (2026-06-20; revised 2026-06-21 — full-autonomy: P04-T3 human-check → green-gated auto-push, runbook agent-driven)
