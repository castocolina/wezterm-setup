...
### Keybindings on macOS — keep-both rationale (D-08)

This is a **durable design note, not a verification step** — it records why the curated key
table ships BOTH a `SUPER` chord and a `Ctrl+Shift` chord for the same actions on macOS, so the
decision is not re-litigated.

- **`SUPER` maps to Cmd on macOS.** WezTerm maps the single `"SUPER"` token to the **Command**
  key on macOS and to the Super/Win key on Linux, so `Cmd+T` (new tab), `Cmd+W` (close tab),
  `Cmd+K` (clear screen + scrollback), and `Cmd++`/`Cmd+-`/`Cmd+0` (font zoom) all work natively
  on macOS from the same source (`config/wezterm-setup/keybindings.lua` — `M.super_mod = "SUPER"`).
- **The `Ctrl+Shift` chords are INTENTIONAL cross-platform fallbacks, kept on macOS.** Many Linux
  WMs/DEs grab the Super key (e.g. Pop!_OS/GNOME bind `Super+W`/`Super+T`/`Super+K`), so a
  `SUPER`-only chord never reaches WezTerm there. The terminal-standard `Ctrl+Shift` variants
  (clear, close-tab, tab navigation) are the reliable fallback and are **kept on macOS too** —
  `wez keys` lists BOTH, which is truthful, not a conflict (they fire the SAME action).
- **Keep-both is a LOCKED decision with NO behavior change (D-08).** Dropping the macOS
  `Ctrl+Shift` fallbacks via `target_triple` gating (platform-specific key tables) is an
  explicitly **deferred idea, not done** — the keep-both set ships as-is on both platforms.
...
| `wez keys --json` | DIAG-04 | **FIXED in 07.1 (Plan 07.1-02)** | Historical root cause (07-05 §4): exited 1, empty stdout, traceback `module 'dkjson' not found` — `keys.lua` did a bare `require("dkjson")` but the vendored module is bundled as `cli.vendor.dkjson`. **CROSS-PLATFORM** (same on dev launcher AND installed binary; Linux baseline was equally broken) — never a macOS divergence. **Fixed 07.1-02:** the `--json` branch now does the project's dual-resolve `pcall(require, "cli.vendor.dkjson")` (bare fallback + unavailable-encoder guard), pinned by a parse regression. NOTE: the **shipped v1.0.0 binary still carries the bug** — the fix reaches end users only via a NEW release |
...
| 5 | `wez keys --json` (DIAG-04, 07-05 §4) | Exited **1**, empty stdout, traceback `module 'dkjson' not found … no module 'dkjson' in luastatic bundle` | `wez keys --json` prints a jq-valid JSON document, exit 0 | **NOT macOS-specific** — `keys.lua` did a bare `require("dkjson")` but the vendored module is bundled as `cli.vendor.dkjson`; failed identically on the dev launcher AND the installed binary (Linux baseline equally broken). A require-path bug, not a parity divergence | **FIXED in 07.1 (Plan 07.1-02):** the `--json` branch now does the dual-resolve `pcall(require, "cli.vendor.dkjson")` (bare fallback + unavailable-encoder guard, mirroring `cli/lib/shell.lua`), pinned by a parse regression. Cross-platform fix. NOTE: the SHIPPED v1.0.0 binary still carries the bug — the fix reaches end-user binaries only via a NEW release |
| 6 | `wez scene new` (ad-hoc scene creation, §6) | Failed `error: mux returned a non-numeric pane id` UNLESS the `wezterm` CLI is on PATH | `wez scene new` builds the scene (Linux ships `wezterm` on PATH by default) | **macOS-specific parity divergence** — `wez scene` shells out to `wezterm` *by name*, but the macOS `WezTerm.app` bundle keeps the CLI at `WezTerm.app/Contents/MacOS/wezterm` and does NOT add it to PATH (Linux installs typically do). Workaround proven LIVE: add `WezTerm.app/Contents/MacOS` to PATH (or symlink `wezterm` into `~/.local/bin`) → `wez scene new --layout grid` then builds the 2×2 grid correctly (sec6-scene-grid.png). Does NOT block the SCEN flips: the `scene launch` error/exit-code contract + verify-macos §4 passed, and `scene new` works once PATH is wired (proven in §6). | **FIXED in 07.1 (Plan 07.1-01):** `install_macos` now symlinks `~/.local/bin/wezterm` → the bundle CLI (`~/Applications/WezTerm.app/Contents/MacOS/wezterm`, sudo-free, mirroring `install_linux`), so `wez scene new`/`launch` resolve `wezterm` out of the box on a clean macOS install. Existing macOS installs pick it up on the next `make install` / `wez update` |
